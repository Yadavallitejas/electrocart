...
from django.contrib.auth.decorators import login_required  # Import login_required for user authentication
...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:  # Check if the user is authenticated
        ...
        is_cart_item_exists = CartItem.objects.filter(product=product, user=current_user).exists()  # Check if the cart item already exists

        if is_cart_item_exists:
            cart_items = CartItem.objects.filter(product=product, user=current_user)  # Get the cart items queryset
            ...
            cart_item = CartItem.objects.create(product=product, quantity=1, user=current_user)  # Create a new cart item
            if len(product_variation) > 0:
...
def remove_cart(request, product_id, cart_item_id):
    ...
    try:
        if request.user.is_authenticated:  # Check if the user is authenticated
            cart_item = CartItem.objects.get(product=product, user=request.user, id=cart_item_id)  # Get the cart item by id
        else:
            cart = Cart.objects.get(cart_id=_cart_id(request))  # Get the cart using the session key
...
@login_required(login_url='login')  # Require user to be logged in to access checkout
